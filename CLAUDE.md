...
This is a **design-handoff bundle**, not an application. It contains one self-contained HTML prototype of a marketing landing page for **PULPA** (a cold-pressed juice brand), plus a comprehensive spec describing how to rebuild it.
...
- `README.md` — **the authoritative handoff spec.** It documents every section, design token, animation, responsive breakpoint, and accessibility gap in exhaustive detail. Treat it as the source of truth; read it before making design changes.
- `assets/img/03-pulpa-splash.jpg`, `03-pulpa-bottle.jpg` — the two product photos referenced by `index.html` (large, ~2 MB each; uncompressed).

There is nothing to build or test. To preview, open `index.html` directly in a browser (`open index.html`).

## The core task

Per `README.md`, the intended workflow is to **recreate this HTML design inside a target codebase's existing environment** (React/Next.js, Vue, Astro, Shopify Liquid, etc.) using that codebase's component patterns, styling system, and image pipeline — **not** to ship the raw HTML file. This repo currently contains no such target environment; if one is introduced, the raw prototype becomes reference-only.

This is a **high-fidelity** design. The intentional "wonkiness" — rotated cards, hard offset shadows (no blur), tilted marquee, per-word headline colors, spinning stickers — **is the design and must be preserved pixel-for-pixel.** Colors, copy, spacing, and rotations in `README.md` are final.
